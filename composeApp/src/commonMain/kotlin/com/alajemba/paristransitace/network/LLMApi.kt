...
class LLMApi(
    ...
    private val scenarioSystemPrompt = { transitRulesJson: String, isFrench: Boolean ->
        ...
        """
            ...
            $transitRulesJson
            Generate one or more Paris transit scenarios using ONLY this data.
    ...
    suspend fun generateScenario(transitRulesJson: String, language: GameLanguage): ApiResponse<List<Scenario>> {
        try {
            scenarioGenerationAgent = AIAgent(
                promptExecutor = simpleGoogleAIExecutor(apiKey),
                agentConfig = AIAgentConfig.withSystemPrompt(
                    llm = GoogleModels.Gemini2_5FlashLite,
                    prompt = scenarioSystemPrompt(transitRulesJson, language == GameLanguage.FRENCH),
            ...
            try {
                val scenarios = json.decodeFromString(ListSerializer(Scenario.serializer()), agentResponse)
